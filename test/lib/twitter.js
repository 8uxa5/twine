...
describe('the twitter module', () => {
	...
	it('should reject on invalid credentials', async () => {
		...
		await expect(twitter.get('/api')).rejectedWith('Invalid Twitter credentials');
		axios.get.restore();
	});
	...
	it('should reject on rate limit error', async () => {
		...
		await expect(twitter.get('/api')).rejectedWith('Twitter rate limit reached');
		axios.get.restore();
	});
	...
	it('should reject on other errors', async () => {
		...
		await expect(twitter.get('/api')).rejectedWith('Twitter:');
		axios.get.restore();
	});
});
