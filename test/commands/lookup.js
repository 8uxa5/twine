...
describe('the lookup module', () => {
	...
	context('users', () => {
		beforeEach(() => {
			...
			sinon.stub(Twitter.prototype, 'get').callsFake((url) => {
				let response = url.slice(url.indexOf('=') + 1).split(',').map((n) => ({ screen_name: n }));
				return Promise.resolve(response);
			});
		});

		it('should lookup users piped to stdin', (done) => {
			let stdin = new ReadableMock(
				[
					'foo\n',
					'bar\n'
				],
		...
		it('should lookup more than 100 users piped to stdin', (done) => {
			...
			lookup.users('twine-test', null, { stdin, stdout });
			stdout.on('finish', () => {
				expect(JSON.parse(stdout.data)).to.deep.equal(users.map((u) => ({ screen_name: u })));
				done();
			});
		});
		...
		it('should reject on error', async () => {
			...
			sinon.stub(Twitter.prototype, 'get').rejects(new Error('Test Error'));
			let stdout = new WritableMock();
